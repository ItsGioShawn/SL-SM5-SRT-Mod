...
for index, RCType in ipairs(RadarCategories.Types) do
	...
	t[#t+1] = Def.RollingNumbers{
		Font="_wendy monospace numbers",
		InitCommand=cmd(zoom,0.35; horizalign, right; Load, "RollingNumbersEvaluationB"),
		BeginCommand=function(self)
	...
	t[#t+1] = LoadFont("_miso")..{
		Text="/",
		InitCommand=cmd(diffuse,color("#5A6166"); zoom, 1.55; horizalign, right),
		BeginCommand=function(self)
			self:y((index-1)*35 + 57)
			self:x( ((player == PLAYER_1) and -166.5) or 230 )
		end
	}

	-- possible value
	t[#t+1] = LoadFont("_wendy monospace numbers")..{
		InitCommand=cmd(zoom,0.35; horizalign, right),
		BeginCommand=function(self)
			self:y((index-1)*35 + 53)
